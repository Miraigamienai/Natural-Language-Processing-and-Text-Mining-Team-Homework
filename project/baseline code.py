...
FEATURES = [
    ...
    "scoreDiff", "scoreSelf", "scoreOther", "side"
    ]
...
class RallyDataset(Dataset):
    def __init__(self, X, Y_list, Y_type_list, L):
    ...
    def __getitem__(self, i): return self.X[i], [y[i] for y in self.Y], self.L[i]
...
class InferDataset(Dataset):
    def __init__(self, data, k, cats):
        ...
        for rid, g in data.groupby("rally_uid", sort=False):
            ...
            Xg = encode_frame(g, FEATURES, cats)
...
class MultiTaskLSTM(nn.Module):
    def __init__(self, num_tokens_per_feature, n_act, n_pt, emb_dim=16, hidden=128, num_layers=1, dropout=0.2):
        ...
        # self.act_change_list = nn.ModuleList([nn.Linear(hidden*(i+1), hidden) for i in range(ACT_SIZE)])
        # self.pt_change_list = nn.ModuleList([nn.Linear(hidden*(i+1), hidden) for i in range(PT_SIZE)])
        self.rly_change_list = nn.ModuleList([nn.Linear(hidden*(i+1), hidden) for i in range(RLY_SIZE)])
    ...
    def forward(self, X, lengths):
        ...
        rly_o = self.drop(nn.functional.gelu(self.rly_change_list[min(RLY_SIZE, T)-1](o[:,:min(RLY_SIZE, T)].view(B, -1))))
...
def main(args):
    ...
    def build_train_dataset(data, k):
        ...
        return RallyDataset(np.stack(X_list), Y_lists, Y_types, L_list)
    ...
    cnt_weights = {
        'act_w':torch.sqrt(torch.sqrt(torch.sqrt(torch.tensor(act_counts/sum(act_counts), dtype=torch.float32).to(device)))),
        'pt_w':torch.sqrt(torch.sqrt(torch.sqrt(torch.tensor(pt_counts/sum(pt_counts), dtype=torch.float32).to(device)))),
    }
...
if __name__ == "__main__":
    ...
    ap.add_argument("--lr", type=float, default=1e-3 *2)
